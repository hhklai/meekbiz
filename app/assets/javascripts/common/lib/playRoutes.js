 ...
 * playRoutes.Application.login().post({user:'username', password:'secret'}).then(function(response) {
 *   ...
 * )};
 * }}}
...
define(['angular', 'jsRoutes'], function (angular, jsRoutes) {
    ...
    var mod = angular.module("common.lib.playRoutes", ['common.lib.cookie']);
    mod.service("playRoutes", ["$http", "cookieFactory", function ($http, cookieFactory) {
        /**
        ...
        var wrapHttp = function (playFunction) {
            return function (/*arguments*/) {
                ...
                var url = meekbiz.settings.backendHost + routeObject.url;
                var res = {
                    method: httpMethod,
                    url: url,
                    relativeUrl: routeObject.url,
                    absoluteUrl: routeObject.absoluteURL,
                    webSocketUrl: routeObject.webSocketURL
                };
                res[httpMethod] = function (obj) {
                    var xsrfToken = cookieFactory.getCookie('XSRF-TOKEN');
                    var config = {};
                    if (xsrfToken) {
                        config.headers = {
                    ...
                    if (['get', 'delete', 'head', 'jsonp'].indexOf(httpMethod) != -1) {
                        return $http[httpMethod](url, config);
                    } else {
                        return $http[httpMethod](url, obj, config);
                    }
                };
                return res;
            };
